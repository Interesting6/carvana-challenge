...
import img.ImgArr2Tensor as ImgA2T
...
class TrainImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = img.resize(self.input_img_resize, Image.ANTIALIAS) # 将原图resize成网络输入大小，Image.ANTIALIAS 高质量
        img = np.asarray(img.convert("RGB"), dtype=np.float32)

        # Pillow reads gifs
        mask = Image.open(self.y_train_masks[index])
        mask = mask.resize(self.output_img_resize, Image.ANTIALIAS) # mask直接resize成388*388了
        mask = np.asarray(mask.convert("L"), dtype=np.float32)  # GrayScale

        if self.X_transform:  # 针对图像的增强
            img, mask = self.X_transform(img, mask)

        if self.y_transform: # 针对标签的增强
            img, mask = self.y_transform(img, mask)

        img = ImgA2T.image_to_tensor(img)
        mask = ImgA2T.mask_to_tensor(mask, self.threshold)
        return img, mask
