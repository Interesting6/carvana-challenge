...
os.environ["KAGGLE_USER"] = "interesting6"
os.environ["KAGGLE_PASSWD"] = "Commoncode1"
...
class DatasetFetcher:
    ...
    def download_dataset(self, hq_files=True):
        ...
        is_datasets_present = True  # 数据集是否存在flag

        # If the folders already exists then the files may already be extracted
        # This is a bit hacky but it's sufficient for our needs  检测数据集是否存在
        for dir_path in datasets_path:
            if not os.path.exists(dir_path):
                is_datasets_present = False

        if not is_datasets_present: # 不存在的话，就下载
            # Put your Kaggle user name and password in a $KAGGLE_USER and $KAGGLE_PASSWD env vars respectively
            # downloader = KaggleDataDownloader(os.getenv("KAGGLE_USER"), os.getenv("KAGGLE_PASSWD"), competition_name)
            downloader = KaggleDataDownloader("interesting6", os.getenv("Commoncode1"), competition_name)

            for file in files:
                # output_path = downloader.download_dataset(file, destination_path)
                output_path = os.path.join(script_dir, "../../input", file)
    ...
    def get_car_image_files(self, car_image_id, test_file=False, get_mask=False): # 每次只取一种集的一个物体的图
        if get_mask:
    ...
    def get_train_files(self, validation_size=0.2, sample_size=None):
        ...
        if validation_size:
            ids_train_split, ids_valid_split = train_test_split(train_ids, test_size=validation_size)  # 4：1分训练验证集
        else:
